...
export const Template = ({ label, status }) =>
  `<nl-status-badge status="${status}">${sanitize(label)}</nl-status-badge>`;
...
<Canvas>
  <span class="nl-status-badge nl-status-badge--success">Available</span>
  <span class="nl-status-badge nl-status-badge--warning">Waiting list</span>
  <span class="nl-status-badge nl-status-badge--error">Unavailable</span>
</Canvas>
...
<MDXEmbedProvider>
  <Figma title="Status Badge" url="file/gqQhMe3gj4YlC6JrZOWiCv/Components?node-id=102%3A200" />
</MDXEmbedProvider>
